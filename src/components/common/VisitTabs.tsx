...
export function VisitTabs({ visitId, active }: VisitTabsProps) {
  return (
    <nav className="visit-tabs" aria-label="Navegación de visita">
      ...
      <Link className={active === 'medications' ? 'visit-tab active' : 'visit-tab'} to={`/visits/${visitId}/medications`}>
        Medicación
      </Link>
      ...
      <Link className={active === 'process' ? 'visit-tab active' : 'visit-tab'} to={`/visits/${visitId}/process`}>
        Proceso
      </Link>
